...
class ImageSharpeningVisualizer:
    ...
    def __init__(self, figure_size=(24, 4), dpi=180):
        self.figure_size = figure_size
        self.dpi = dpi
        logger.info(
            'ImageSharpeningVisualizer initialized (size=%s, dpi=%d)', figure_size, dpi)
    ...
    def display_sharpening_results(
        ...
        figure, axes = plt.subplots(
            1, 5, figsize=self.figure_size, dpi=self.dpi)
        figure.suptitle(
            f'Image Sharpening Results - {image_filename}', fontsize=16)
        ...
        if figure_save_path:
            ...
            logger.debug(
                'Sharpening comparison figure saved to %s', figure_save_path)
